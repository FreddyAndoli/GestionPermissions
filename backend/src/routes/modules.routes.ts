...
router.get('/', authMiddleware, requirePermission('permissions.read'), async (req: Request, res: Response) => {
  const rows = await db.select().from(modules).where(eq(modules.organizationId, req.user!.organizationId));
  res.json(rows);
});

router.post('/', authMiddleware, requirePermission('permissions.create'), async (req: Request, res: Response) => {
...
router.put('/:id', authMiddleware, requirePermission('permissions.update'), async (req: Request, res: Response) => {
...
router.delete('/:id', authMiddleware, requirePermission('permissions.delete'), async (req: Request, res: Response) => {
  const id = parseInt(req.params.id);
  await db.delete(modules).where(eq(modules.id, id));
  res.json({ message: 'Module deleted' });
});

router.patch('/:id/toggle', authMiddleware, requirePermission('permissions.update'), async (req: Request, res: Response) => {
  const id = parseInt(req.params.id);
